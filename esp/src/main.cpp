#include <Arduino.h>
#include <LittleFS.h> // Używamy LittleFS jako systemu plików w pamięci flash
...
void setup() {
    ...
    if (!LittleFS.begin(true)) { // 'true' oznacza, że sformatuje pamięć, jeśli wystąpi błąd
    ...
    // 3. Tworzymy obiekt naszego loggera i go odpalamy
    myLogger = new SerialLogger(logFile, 115200);
    myLogger->begin();

    // Pierwszy log testowy
    myLogger->info("Logger zainicjalizowany pomyslnie. Start pętli głównej.");
}
